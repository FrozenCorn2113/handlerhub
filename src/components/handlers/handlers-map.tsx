...
export function HandlersMap({
  ...
  const initialCenter = useMemo<[number, number]>(() => {
    ...
    const avgLng = pts.reduce((s: number, f: any) => s + f.geometry.coordinates[0], 0) / pts.length
    const avgLat = pts.reduce((s: number, f: any) => s + f.geometry.coordinates[1], 0) / pts.length
    ...
    const init = async () => {
      ...
      map.on('load', () => {
        ...
        map.on('click', PINS_LAYER, (e: any) => {
          const features = map.queryRenderedFeatures(e.point, { layers: [PINS_LAYER] })
          if (!features.length) return
  ...
  useEffect(() => {
    const map = mapRef.current
    if (!map || !mapReady) return
    map.setPaintProperty(PINS_LAYER, 'circle-radius', highlightedId
      ? ['case', ['==', ['get', 'id'], highlightedId], 11, 7]
      : 7
    )
    map.setPaintProperty(PINS_LAYER, 'circle-opacity', highlightedId
    ...
    <div className="relative h-full w-full overflow-hidden">
      ...
      {handlers.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80">
          ...
          <p className="text-sm font-medium text-warm-gray">No handlers to display</p>
